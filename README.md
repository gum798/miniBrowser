...
```bash
brew install --cask gum798/tap/minibrowser
```

`/Applications`에 설치됩니다. 업데이트는 `brew upgrade --cask minibrowser`, 제거는 `brew uninstall --cask minibrowser`(데이터까지 지우려면 `brew uninstall --zap --cask minibrowser`).

> 직접 서명/노터라이즈한 앱이 아니라 ad-hoc 서명이라, cask가 설치 시 quarantine 플래그를 제거해 바로 실행되게 합니다. 릴리즈 빌드·게시는 `./scripts/release.sh <version> --publish`로 만듭니다(cask의 sha256도 자동 갱신).
