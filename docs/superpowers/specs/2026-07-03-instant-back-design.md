...
- **`EdgeSwipeOverlay`** (수정 소폭)
  ...
  - **라이브 리빌(책장 넘기기)**: 드래그 중 스택의 실제 이전(또는 다음) 웹뷰를
    현재 웹뷰 **아래에 깔아** 실제 화면이 드러나게 한다(사용자 요청으로 범위에 포함,
    2026-07-03). 대상 항목이 placeholder이거나 네이티브 히스토리가 우선이면 기존
    슬라이드+셰브론으로 폴백. 커밋 시 스왑, 취소 시 스냅백 애니메이션 후 밑그림 제거.
    두 손가락 스와이프에도 동일 적용.
